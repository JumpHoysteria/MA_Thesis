...
\caption{Piezo1-mediated influx of extracellular Ca2+ confirmed as mechanotransductory event in MSC. \myworries{NEW ARRANGEMENT: D, C, B, A} \textit{Top:} Schematic drawing of Flowchamber. a.) Calcium-influx in response to fluidic shear pulse of 5s with an amplitude of 7Pa (Data points describe technical replicates, ***p$<$0.0001, ANOVA with Dunnette's multiple comparison test) b.) Same data as in previous graph with time-curve plotted c.) Peak values of fluorescence levels of samples in calcium-free and calcium-containing medium. (n = 3, *p $<$ 0.05, Student's t-test) d.) Representative measurement of impulse response with two different media (Pooled data of 4 technical replicates, Data is mean $\pm$ SEM)}
\label{fig:Calcium}
\end{figure}
